...
class FPN(nn.Module):
    def __init__(self, block_expansion=1, backbone="resnet50"):
        super().__init__()
        ...
        #         self.feature_extractor.layer3.requires_grad = True
        #         self.feature_extractor.layer4.requires_grad = True
        # two more layers conv6 and conv7 on the top of layer4 (if backbone is resnet18)

        self.conv6 = nn.Conv2d(
            2048 * block_expansion, 64 * block_expansion, kernel_size=3, stride=2, padding=1
        )
        ...
        self.latlayer1 = nn.Conv2d(
            2048 * block_expansion, 64 * block_expansion, kernel_size=1, stride=1, padding=0
        )
        self.latlayer2 = nn.Conv2d(
            1024 * block_expansion, 64 * block_expansion, kernel_size=1, stride=1, padding=0
        )
